...
# ============================================================
# ---- Online SVGP for axis-wise 1D inputs (batched) ----------
# ============================================================
class OnlineSVGPBatchAxiswise1D:
    """Independent SVGPs where each output uses its own scalar input.

    This is useful for models such as:
      ax = g(Fx), ay = g(Fy), az = g(Fz)

    Conventions:
      - X: (N, T) with one scalar input per output dimension
      - Y: (N, T) with one scalar target per output dimension
      - Internally the GP sees X as (T, N, 1) to keep the outputs batched
        and run them efficiently on GPU in a single forward pass.
    ...
    def __init__(
        ...
        # Raw data in original scale
        self.X: torch.Tensor | None = None   # (N,T)
        self.Y: torch.Tensor | None = None   # (N,T)

        # Fixed normalization stats after first fit
        self.X_mean: torch.Tensor | None = None  # (T,)
        self.X_std: torch.Tensor | None = None   # (T,)
        self.Y_mean: torch.Tensor | None = None  # (T,)
        self.Y_std: torch.Tensor | None = None   # (T,)

        # Normalized data
        self.Xn: torch.Tensor | None = None  # (N,T)
    ...
    def _validate_xy(self, X: torch.Tensor, Y: torch.Tensor):
        if X.ndim != 2 or X.shape[1] != self.output_dim:
            raise ValueError(f"X must be (N,{self.output_dim}), got {tuple(X.shape)}")
    ...
    def _x_model_view(self, Xn: torch.Tensor) -> torch.Tensor:
        # (N,T) -> (T,N,1)
        return Xn.transpose(0, 1).contiguous().unsqueeze(-1)
    ...
    def _init_model(self):
        ...
        Xn_model = self._x_model_view(self.Xn)  # (T,N,1)
        rows = torch.arange(self.output_dim, device=self.device)[:, None]
        perm = torch.stack([torch.randperm(N, device=self.device)[:M] for _ in range(self.output_dim)], dim=0)
        inducing_points = Xn_model[rows, perm].contiguous()  # (T,M,1)
    ...
    def _train_steps(self, steps: int):
        ...
        for _ in range(int(steps)):
            idx = torch.randint(0, N, (bs,), device=self.device)
            xb = self._x_model_view(self.Xn[idx])  # (T,bs,1)
            yb = self.Yn[idx].transpose(0, 1).contiguous()  # (T,bs)
    ...
    def predict_torch(self, X: torch.Tensor, return_var: bool = True) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if X.ndim != 2 or X.shape[1] != self.output_dim:
            raise ValueError(f"X must be (B,{self.output_dim}), got {tuple(X.shape)}")

        self._ensure_stats()
        Xn = (X - self.X_mean) / self.X_std
        Xn_model = self._x_model_view(Xn)  # (T,B,1)

        with _INFER_CTX():
            latent = self.model(Xn_model)
            meanTB = latent.mean  # (T,B)
            mean = (meanTB.transpose(0, 1) * self.Y_std) + self.Y_mean  # (B,T)

            if return_var:
                with gpytorch.settings.fast_pred_var():
                    pred = self.likelihood(latent)
                    varTB = pred.variance  # (T,B)
                var = varTB.transpose(0, 1) * (self.Y_std ** 2)
                return mean, var
            else:
                return mean, torch.zeros_like(mean)
    ...
    def load_from_bundle(self, bundle: dict):
        ...
        if key not in model_state:
            raise RuntimeError("Checkpoint missing inducing_points in model_state.")
        inducing_shape = model_state[key].shape  # (T,M,1)
        ...
        if D_ckpt != 1:
            raise RuntimeError(f"Expected axis-wise 1D inducing points, got D={D_ckpt}")
        if T_ckpt != self.output_dim:
            self.output_dim = int(T_ckpt)

        inducing_dummy = torch.zeros((self.output_dim, M_ckpt, 1), device=self.device, dtype=self.dtype)

        self.likelihood = gpytorch.likelihoods.GaussianLikelihood(batch_shape=torch.Size([self.output_dim])).to(self.device)
        self.model = BatchedSparseGPModel(inducing_dummy, num_outputs=self.output_dim, kernel=self.kernel, ard_dims=1).to(self.device)
